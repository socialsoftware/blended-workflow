...
public class OldTaskWorkItem extends OldTaskWorkItem_Base {
	...
	public OldTaskWorkItem(OldBWInstance bwInstance, Activity task) {
		...
		task.getPreConditionSet().stream()
				.forEach((cond) -> cond.assignAttributeInstances(this, ConditionType.PRE_CONDITION));
		createInputWorkItemArguments();

		task.getPostConditionSet().stream()
				.forEach((c) -> c.assignAttributeInstances(this, ConditionType.POS_CONDITION));
	...
	public void notifyCompleted() {
		...
		getBwInstance().getLog()
				.addLogRecords(new LogRecord(date, "Completed", "[ACTIVITY] " + getID(), getUser().getID()));
		// BlendedWorkflow.getInstance().getWorkListManager()
	...
	public void notifySkipped() {
		...
		String date = dateFormat.format(Calendar.getInstance().getTime());
		getBwInstance().getLog()
				.addLogRecords(new LogRecord(date, "Skipped", "[ACTIVITY] " + getID(), getUser().getID()));
		// BlendedWorkflow.getInstance().getWorkListManager()
		// .notifyCompletedWorkItem(this);
	}
