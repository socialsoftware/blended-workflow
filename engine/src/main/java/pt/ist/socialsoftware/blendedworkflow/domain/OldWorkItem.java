...
public abstract class OldWorkItem extends OldWorkItem_Base {
	...
	public void createInputWorkItemArguments() {
		for (OldAttributeInstance attributeInstance : getInputAttributeInstancesSet()) {
			OldWorkItemArgument workItemArgument = new OldWorkItemArgument(attributeInstance,
					attributeInstance.getValue(), attributeInstance.getState());
	...
	public void notifyWorkItemDataChanged(Boolean isPreTask) {
		...
		if (isPreTask) {
			for (OldAttributeInstance attributeInstance : getInputAttributeInstancesSet()) {
				for (OldWorkItem workItem : attributeInstance.getPreConstraintWorkItemsSet()) {
					notifyWorkItems.add(workItem);
				}
				for (OldWorkItem workItem : attributeInstance.getContraintViolationWorkItemsSet()) {
					notifyWorkItems.add(workItem);
				}
			}
		}
