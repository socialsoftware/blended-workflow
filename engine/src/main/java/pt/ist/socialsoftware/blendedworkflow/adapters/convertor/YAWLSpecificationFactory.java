...
public class YAWLSpecificationFactory {
	...
	public String parseYAWLSpecificationFactory(Specification bwSpecification) throws BWException {
	...
	private String metaData() {
		String metaData = "<specification uri=\"" + this.specificationURI + "\">" + "<metaData>" + "<title>"
				+ this.bwSpecification.getName() + "</title>" + "<creator>" + this.bwSpecification.getAuthor()
				+ "</creator>" + "<description>" + this.bwSpecification.getDescription() + "</description>"
				+ "<version>" + this.bwSpecification.getVersion() + "</version>" + "<persistent>false</persistent>"
				+ "<identifier>" + this.bwSpecification.getUID() + "</identifier>" + "</metaData>";
		return metaData;
	}
	...
	private String localVariables() {
		...
		for (Attribute attribute : dataModel.getAttributeBasicSet()) {
			...
			String localVariable = "<localVariable>" + "<index>" + lovalVariablesIndex + "</index>" + "<name>"
					+ entityName + "_" + attributeName + "_State" + "</name>" + "<type>" + "string" + "</type>"
					+ "<namespace>http://www.w3.org/2001/XMLSchema</namespace>" + "<initialValue>" + DataState.UNDEFINED
					+ "</initialValue>" + "</localVariable>";

			lovalVariablesIndex++;
			localVariable += "<localVariable>" + "<index>" + lovalVariablesIndex + "</index>" + "<name>" + entityName
					+ "_" + attributeName + "</name>" + "<type>" + type + "</type>"
					+ "<namespace>http://www.w3.org/2001/XMLSchema</namespace>" + "<initialValue>" + value
					+ "</initialValue>" + "</localVariable>";
			lovalVariablesIndex++;
	...
	private String processControlElements() {
		...
		for (Activity task : taskModel.getActivitySet()) {
			...
			completedMappings = "";
			if (firstTask) {
				element += "<processControlElements>" + "<inputCondition id=\"InputCondition_1\">" + "<flowsInto>"
						+ "<nextElementRef id=\"" + taskID + "\" />" + "</flowsInto>" + "</inputCondition>";
			...
			if (task.getNextActivitySet().size() > 0) {
				...
				element += "<flowsInto>" + "<nextElementRef id=\"" + "OutputCondition_2" + "\" />" + "</flowsInto>";
			}

			element += "<join code=\"" + joinCode + "\" />" + "<split code=\"" + splitCode + "\" />";
			...
			if (attributes.size() > 0) {
				...
				for (Attribute attribute : attributes) {
					...
					startingMappings += "<mapping>" + "<expression query=\"&lt;" + entityName + "_" + attributeName
							+ "_State" + "&gt;{/" + this.specificationURI + "/" + entityName + "_" + attributeName
							+ "_State" + "/text()}&lt;/" + entityName + "_" + attributeName + "_State" + "&gt;\" /> "
							+ "<mapsTo>" + entityName + "_" + attributeName + "_State" + "</mapsTo>" + "</mapping>";

					startingMappings += "<mapping>" + "<expression query=\"&lt;" + entityName + "_" + attributeName
							+ "&gt;{/" + this.specificationURI + "/" + entityName + "_" + attributeName
							+ "/text()}&lt;/" + entityName + "_" + attributeName + "&gt;\" /> " + "<mapsTo>"
							+ entityName + "_" + attributeName + "</mapsTo>" + "</mapping>";
			...
			// Complete Mapping
			attributes = task.getPostConditionSet().stream().flatMap((c) -> c.getAttributeSet().stream())
					.collect(Collectors.toSet());
			if (attributes.size() > 0) {
				...
				for (Attribute attribute : attributes) {
					...
					completedMappings += "<mapping>" + "<expression query=\"&lt;" + entityName + "_" + attributeName
							+ "_State" + "&gt;{/" + taskName + "/" + entityName + "_" + attributeName + "_State"
							+ "/text()}&lt;/" + entityName + "_" + attributeName + "_State" + "&gt;\" /> " + "<mapsTo>"
							+ entityName + "_" + attributeName + "_State" + "</mapsTo>" + "</mapping>";

					completedMappings += "<mapping>" + "<expression query=\"&lt;" + entityName + "_" + attributeName
							+ "&gt;{/" + taskName + "/" + entityName + "_" + attributeName + "/text()}&lt;/"
							+ entityName + "_" + attributeName + "&gt;\" /> " + "<mapsTo>" + entityName + "_"
							+ attributeName + "</mapsTo>" + "</mapping>";
				}
				completedMappings += "</completedMappings>";
			...
			// footer
			element += "<resourcing>" + "<offer initiator=\"user\" />" + "<allocate initiator=\"user\" />"
					+ "<start initiator=\"user\" />" + "</resourcing>" + "<decomposesTo id=\"" + taskName + "\" />"
					+ "</task>";
			processControlElements += element;
		}
	...
	private String inputOutputParam() {
		...
		for (Activity task : taskModel.getActivitySet()) {
			...
			for (Attribute attribute : attributes) {
				...
				inputParam += "<inputParam>" + "<index>" + inputOutputParamIndex + "</index>" + "<name>" + entityName
						+ "_" + attributeName + "_State" + "</name>" + "<type>" + "string" + "</type>"
						+ "<namespace>http://www.w3.org/2001/XMLSchema</namespace>" + "</inputParam>";
			...
			// Output Parameters
			attributes = task.getPostConditionSet().stream().filter(Attribute.class::isInstance)
					.map(Attribute.class::cast).collect(Collectors.toSet());
			for (Attribute attribute : attributes) {
				String entityName = attribute.getEntity().getName().replaceAll(" ", "");
				String attributeName = attribute.getName().replaceAll(" ", "");
				String type = attribute.getYAWLAttributeType();
				outputParam += "<outputParam>" + "<index>" + inputOutputParamIndex + "</index>" + "<name>" + entityName
						+ "_" + attributeName + "_State" + "</name>" + "<type>" + "string" + "</type>"
						+ "<namespace>http://www.w3.org/2001/XMLSchema</namespace>" + "</outputParam>";
