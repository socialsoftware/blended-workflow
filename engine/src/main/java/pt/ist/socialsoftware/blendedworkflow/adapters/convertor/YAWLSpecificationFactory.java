...
public class YAWLSpecificationFactory {
    ...
    private String localVariables() {
        ...
        for (Attribute attribute : dataModel.getAttributesSet()) {
            ...
            lovalVariablesIndex++;
            localVariable += "<localVariable>" + "<index>" + lovalVariablesIndex
                    + "</index>" + "<name>" + entityName + "_" + attributeName
                    + "</name>" + "<type>" + type + "</type>"
                    + "<namespace>http://www.w3.org/2001/XMLSchema</namespace>"
        ...
        for (Task task : taskModel.getTasksSet()) {
            ...
            if (attributes.size() > 0) {
                ...
                for (Attribute attribute : attributes) {
                    ...
                    startingMappings += "<mapping>" + "<expression query=\"&lt;"
                            + entityName + "_" + attributeName + "_State"
                            + "&gt;{/" + this.specificationURI + "/"
                            + entityName + "_" + attributeName + "_State"
                            + "/text()}&lt;/" + entityName + "_" + attributeName
                            + "_State" + "&gt;\" /> " + "<mapsTo>" + entityName
                            + "_" + attributeName + "_State" + "</mapsTo>"
                            + "</mapping>";

                    startingMappings += "<mapping>" + "<expression query=\"&lt;"
                            + entityName + "_" + attributeName + "&gt;{/"
                            + this.specificationURI + "/" + entityName + "_"
                            + attributeName + "/text()}&lt;/" + entityName + "_"
            ...
            if (attributes.size() > 0) {
                ...
                for (Attribute attribute : attributes) {
                    ...
                    completedMappings += "<mapping>"
                            ...
                            + entityName + "_" + attributeName + "/text()}&lt;/"
                            + entityName + "_" + attributeName + "&gt;\" /> "
                            + "<mapsTo>" + entityName + "_" + attributeName
                            + "</mapsTo>" + "</mapping>";
                }
                completedMappings += "</completedMappings>";
    ...
    private String inputOutputParam() {
        ...
        for (Task task : taskModel.getTasksSet()) {
            ...
            for (Attribute attribute : attributes) {
                ...
                inputParam += "<inputParam>" + "<index>" + inputOutputParamIndex
                        + "</index>" + "<name>" + entityName + "_"
                        + attributeName + "_State" + "</name>" + "<type>"
                        + "string" + "</type>"
                        + "<namespace>http://www.w3.org/2001/XMLSchema</namespace>"
                        + "</inputParam>";
            ...
            for (Attribute attribute : attributes) {
                ...
                String attributeName = attribute.getName().replaceAll(" ", "");
                String type = attribute.getYAWLAttributeType();
                outputParam += "<outputParam>" + "<index>"
                        + inputOutputParamIndex + "</index>" + "<name>"
                        + entityName + "_" + attributeName + "_State"
                ...
                outputParam += "<outputParam>" + "<index>"
                        + inputOutputParamIndex + "</index>" + "<name>"
                        + entityName + "_" + attributeName + "</name>"
                        + "<type>" + type + "</type>"
                        + "<namespace>http://www.w3.org/2001/XMLSchema</namespace>"
                        + "</outputParam>";
                inputOutputParamIndex++;
