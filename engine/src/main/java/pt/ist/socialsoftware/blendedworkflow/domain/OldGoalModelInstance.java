...
public class OldGoalModelInstance extends OldGoalModelInstance_Base {
	 ...
	 */
	public Set<OldMaintainGoal> getAchieveGoalAssociatedMaintainGoals(Goal achieveGoal) {
		...
		for (OldMaintainGoal maintainGoal : getMaintainGoalsSet()) {
			...
			achieveAttributes.addAll(achieveGoal.getSuccessConditionSet().stream().findFirst().get().getAttributeSet());
			achieveEntities.addAll(achieveGoal.getSuccessConditionSet().stream().findFirst().get().getEntities());
			...
			maintainAttributes.addAll(maintainGoal.getMaintainCondition().getAttributeSet());
	...
	public Set<Goal> getAchieveGoalAssociatedAchieveGoals(Goal achieveGoal) {
		...
		for (Goal ag : getGoalSet()) {
			if (!achieveGoal.equals(ag)) {
				...
				Set<Entity> achieveEntities = new HashSet<Entity>();
				achieveAttributes
						.addAll(achieveGoal.getSuccessConditionSet().stream().findFirst().get().getAttributeSet());
				achieveEntities.addAll(achieveGoal.getSuccessConditionSet().stream().findFirst().get().getEntities());
				...
				agAttributes.addAll(ag.getSuccessConditionSet().stream().findFirst().get().getAttributeSet());
				agEntities.addAll(ag.getSuccessConditionSet().stream().findFirst().get().getEntities());
