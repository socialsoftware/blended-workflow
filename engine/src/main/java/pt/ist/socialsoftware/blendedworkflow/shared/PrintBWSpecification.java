...
public class PrintBWSpecification {
	...
	public static void all(String bwSpecificationName)
	...
	private static void init(String bwSpecificationName)
			throws BlendedWorkflowException {
		blendedWorkflow = BlendedWorkflow.getInstance();
		bwSpecification = blendedWorkflow
				.getBWSpecification(bwSpecificationName);
	...
	public static void dataModelInstances(String bwSpecificationName)
			...
			if (entity.getEntityInstancesSet().size() > 0) {
				for (EntityInstance entityInstance : entity
					...
					System.out.println("EntityInstance \""
							+ entityInstance.getID() + "\" is associated with "
							+ entityInstance.getAttributeInstancesSet().size()
							+ " AttributesInstances.");
		...
		for (Relation relation : dataModelInstance.getRelationsSet()) {
			System.out
					.println("Relation \"" + relation.getName() + "\" has "
