...
public class PrintBWSpecification {
	...
	private static void init(String bwSpecificationName) throws BWException {
		...
		bwSpecification = blendedWorkflow.getSpecById(bwSpecificationName).orElse(null);
	...
	public static void bwInstances(String bwSpecificationName) throws BWException {
		init(bwSpecificationName);
		System.out.println("**************************************************************");
		System.out.println("BWInstances:");
		System.out.println("----------------------------------------------------------");
		System.out.println(
				bwSpecification.getName() + " has " + bwSpecification.getOldBwInstancesSet().size() + " instance.");
		System.out.println(bwInstance.getID());
	}

	public static void dataModelTemplate(String bwSpecificationName) throws BWException {
		init(bwSpecificationName);
		System.out.println("**************************************************************");
		System.out.println("DataModel Template");
		System.out.println("----------------------------------------------------------");
		for (Entity entity : dataModelTemplate.getEntitySet()) {
			System.out.println("Entity \"" + entity.getName() + "\" has " + entity.getOldEntityInstanceSet().size()
					+ " Instances");
	...
	public static void dataModelType(String bwSpecificationName) throws BWException {
		init(bwSpecificationName);
		System.out.println("**************************************************************");
	...
	public static void goalModelTemplate(String bwSpecificationName) throws BWException {
		init(bwSpecificationName);
		System.out.println("**************************************************************");
		System.out.println("GoalModel Template");
		System.out.println("----------------------------------------------------------");
		for (Goal goal : goalModel.getGoalSet()) {
			System.out.println("Goal \"" + goal.getName() + "\" has \"" + goal.getOldGoalWorkItemSet().size()
					+ "\" WorkItems and has " + goal.getSubGoalSet().size() + " subgoals.");
			for (Condition ac : goal.getActivationConditionSet()) {
				System.out.println("ActivateCondition: " + ac);
			}
			System.out.println("SucessCondition: " + goal.getSuccessCondition());
			System.out.println("User: " + goal.getUser().getID() + " Role: " + goal.getRole().getName());
		}
	}

	public static void goalModelType(String bwSpecificationName) throws BWException {
		init(bwSpecificationName);
		System.out.println("**************************************************************");
	...
	public static void taskModelTemplate(String bwSpecificationName) throws BWException {
		init(bwSpecificationName);
		System.out.println("**************************************************************");
		...
		for (Activity task : taskModel.getActivitySet()) {
			...
			task.getPreConditionSet().stream().forEach((cond) -> System.out.println(cond));
			System.out.println("Post Condition: ");
			task.getPostConditionSet().stream().forEach((cond) -> System.out.println(cond));
			System.out.println("User: " + task.getUser().getID() + " Role: " + task.getRole().getName());
		}
	}

	public static void taskModelType(String bwSpecificationName) throws BWException {
		init(bwSpecificationName);
		System.out.println("**************************************************************");
	...
	public static void dataModelInstances(String bwSpecificationName) throws BWException {
		init(bwSpecificationName);
		System.out.println("**************************************************************");
		...
		for (Entity entity : dataModelInstance.getEntitySet()) {
			...
			if (entity.getOldEntityInstanceSet().size() > 0) {
				for (OldEntityInstance entityInstance : entity.getOldEntityInstanceSet()) {
					System.out.println("EntityInstance \"" + entityInstance.getID() + "\" is associated with "
							+ entityInstance.getAttributeInstancesSet().size() + " AttributesInstances.");
					for (OldAttributeInstance attributeInstance : entityInstance.getAttributeInstancesSet()) {
						System.out.println("AttributeInstance \"" + attributeInstance.getID() + "\" with value \""
								+ attributeInstance.getValue() + "\" and state " + attributeInstance.getState() + ".");
					}
				}
			}
			System.out.println("----------------------------------------------------------");
		...
		for (RelationBW relation : dataModelInstance.getRelationBWSet()) {
			System.out.println("Relation \"" + relation.getName() + "\" has "
					+ relation.getOldRelationInstanceSet().size() + " instances.");
	...
	public static void workItems(String bwSpecificationName) throws BWException {
		init(bwSpecificationName);
		System.out.println("**************************************************************");
		...
		for (OldWorkItem workitem : bwInstance.getWorkItemsSet()) {
			if (workitem instanceof OldGoalWorkItem) {
				...
				System.out.println("GoalWorkItem \"" + workitem.getID() + "\" is " + goalWorkItem.getState()
						+ " and is associated with " + workitem.getOutputAttributeInstancesSet().size()
						+ " attributeInstances.");
			}

			if (workitem instanceof OldTaskWorkItem) {
				OldTaskWorkItem taskWorkItem = (OldTaskWorkItem) workitem;
				System.out.println("GoalWorkItem \"" + workitem.getID() + "\" is " + taskWorkItem.getState()
						+ " and is associated with " + workitem.getOutputAttributeInstancesSet().size()
						+ " attributeInstances.");
			}
		}
	...
	public static void workItemsWithAttributtes(String bwSpecificationName) throws BWException {
		init(bwSpecificationName);
		System.out.println("**************************************************************");
		...
		for (OldWorkItem workitem : bwInstance.getWorkItemsSet()) {
			...
			System.out.println("Input: Associated with " + workitem.getInputAttributeInstancesSet().size()
					+ " attributeInstances.");
			if (workitem.getInputAttributeInstancesSet().size() > 0) {
				for (OldAttributeInstance attributeInstance : workitem.getInputAttributeInstancesSet()) {
					System.out.println("EI" + attributeInstance.getEntityInstance().getID() + "AttributeInstance \""
							+ attributeInstance.getID() + "\" with value \"" + attributeInstance.getValue()
							+ "\" and state " + attributeInstance.getState() + ".");
				}
			}

			System.out.println("Output: Associated with " + workitem.getOutputAttributeInstancesSet().size()
					+ " attributeInstances.");
			if (workitem.getOutputAttributeInstancesSet().size() > 0) {
				for (OldAttributeInstance attributeInstance : workitem.getOutputAttributeInstancesSet()) {
					System.out.println("EI: " + attributeInstance.getEntityInstance().getID() + " AttributeInstance \""
							+ attributeInstance.getID() + "\" with value \"" + attributeInstance.getValue()
							+ "\" and state " + attributeInstance.getState() + ".");
				}
