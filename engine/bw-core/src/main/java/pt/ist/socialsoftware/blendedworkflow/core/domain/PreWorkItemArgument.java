...
public class PreWorkItemArgument extends PreWorkItemArgument_Base {
	...
	public boolean preHolds(Set<PostWorkItemArgument> postWorkItemArguments) {
		...
		Set<ProductInstance> productInstances = postWorkItemArguments.stream().filter(
				wia -> wia.getDefProductCondition().getSourceOfPath() == getDefPathCondition().getSourceOfPath())
