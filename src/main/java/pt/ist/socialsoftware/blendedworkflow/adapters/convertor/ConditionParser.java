...
public class ConditionParser {
	...
	protected Attribute parseExistsAttributeConditionArgs(
		...
		elementName.append(existsAttributeCondition.substring(startArgs,
				endArgs));
	...
	protected Condition parseCompareAttributeToCondition()
		...
		elementName.append(compareAttributeToString.substring(startArgs,
				subToken));
		String[] elementArr = elementName.toString().split("\\.");
		Entity entity = parseEntity(elementArr);
		...
		elementTo.append(compareAttributeToString.substring(subToken + 1,
				endArgs).trim());
	...
	protected Condition parseForAllCondition() throws BlendedWorkflowException {
		...
		// Parse condition
		subConditionString.append(forAllString.substring(subToken + 1, endArgs)
				.trim());
	...
	protected Condition parseExistsOneCondition()
		...
		Relation relation = parseRelation(elementArr[1]);
		// Parse condition
		subConditionString.append(exitsOneString.substring(subToken + 1,
				endArgs).trim());
