...
public class YAWLAdapter extends InterfaceBWebsideController {
	...
	public static YAWLAdapter getInstance() {
		if(instance == null) {
	...
	// Specifications
	public boolean loadSpecification(String specID, String spec, boolean yawlSpec) {
	...
	public boolean unloadSpecification(String specID) throws BlendedWorkflowException {
		if(specID == null) {
	...
	// Cases
	public String launchCase(String caseID, boolean isActivityCase) {
		// get the loaded cases
		log.info("input "+ caseID);
		List<SpecificationData> specList = getLoadedSpecs();
		if(specList == null) {
			log.error("Null specification list");
			return null;
