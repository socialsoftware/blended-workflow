...
public class PreTaskForm extends VerticalLayout {
	...
	private void setWorkItemArgumentValue(int index, String value) {
		...
		taskWorkItem.getPreConstrainWorkItemArguments().get(index).setValue(value);
		taskWorkItem.getPreConstrainWorkItemArguments().get(index).setState(DataState.DEFINED);
		Transaction.commit();
	}
	...
	private void getOutputData() {
		...
		Boolean first = true;
		for (WorkItemArgument workItemArgument : taskWorkItem.getPreConstrainWorkItemArguments()) {
