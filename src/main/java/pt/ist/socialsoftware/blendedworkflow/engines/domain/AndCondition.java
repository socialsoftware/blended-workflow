...
public class AndCondition extends AndCondition_Base {
	...
	Condition cloneCondition(GoalModelInstance goalModelInstance) {
		return new AndCondition(getConditionOne().cloneCondition(
				goalModelInstance), getConditionTwo().cloneCondition(
				goalModelInstance));
	}
	...
	Condition cloneCondition(TaskModelInstance taskModelInstance) {
		return new AndCondition(getConditionOne().cloneCondition(
				taskModelInstance), getConditionTwo().cloneCondition(
				taskModelInstance));
	}
	...
	public TripleStateBool evaluate(GoalWorkItem goalWorkItem,
			ConditionType conditionType) {
		return getConditionOne().evaluate(goalWorkItem, conditionType).AND(
				getConditionTwo().evaluate(goalWorkItem, conditionType));
	}

	@Override
	public TripleStateBool evaluateWithWorkItem(GoalWorkItem goalWorkItem,
			ConditionType conditionType) {
		return getConditionOne().evaluateWithWorkItem(goalWorkItem,
				conditionType).AND(
				getConditionTwo().evaluateWithWorkItem(goalWorkItem,
						conditionType));
	}
	...
	public TripleStateBool evaluateWithDataModel(EntityInstance entityInstance,
		...
		return getConditionOne().evaluateWithDataModel(entityInstance,
				goalWorkItem, conditionType).AND(
				getConditionTwo().evaluateWithDataModel(entityInstance,
