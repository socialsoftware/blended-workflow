...
public class ManageSplit {
	public static void goals(GoalModel model, String name, CommandSplit split) throws ParameterException, ValueException  {
		//CHECKS
		List<String> successConditions1 = Arrays.asList(split.successConditions.split(";")); //different elements
		List<List<String>> successConditionsSplitted1 = new ArrayList<>(successConditions1.size());
		for (String suc : successConditions1) {
			List<String> part = Arrays.asList(suc.split(","));
			successConditionsSplitted1.add(part);
		}
		if (successConditionsSplitted1.size() < 1) throw new ParameterException("Parameter not valid: One existing goal expected to be splitted");
		//List<String> serializedSuccessConditions = serializedSuccessConditions(successConditionsSplitted1);
		...
		List<EObject> succList1 = checkSuccessConditions(goal.getSuccessConditions(), successConditionsSplitted1);
		if (succList1 == null) {
		...
		manageSuccessConditions(goal.getSuccessConditions(), newGoal1, newGoal2, succList1);				
		List<List<String>> successConditionsSplitted2 = getSucessConditionsSplitted(newGoal2);
		...
		Optional<ValueException> rel1 = manageRelatives(model, goal, newGoal1, newGoal2, type);
		if (rel1.isPresent()) throw rel1.get();
	...
	/**
	 * I just need to copy all the success conditions, but taking into account that I don't want any of them without a SUC value. At least SUC(Nothing)
	 * @param goalSuccessConditions
	 * @param newGoal
	 * @param succList
	 */
	public static void manageSuccessConditions(EList<EObject> goalSuccessConditions, Goal newGoal1, Goal newGoal2, List<EObject> succList) {
		newGoal1.getSuccessConditions().addAll(copyAll(succList));
		goalSuccessConditions.removeAll(succList); //we remove all the ones we are using
		if (goalSuccessConditions.size() == 0) { //I don't want that to be empty for any goal
		...
		newGoal2.getSuccessConditions().addAll(copyAll(goalSuccessConditions)); //the remaining for the second new goal
	}
	
	/**
	 * I just need to call to the engine to know given a SUC which conditions it depends on 
	 * That way I will copy such ACTs with the corresponding SUCs
	 * @param goal
	 * @param newGoal
	 * @param serializedSuccessConditions
	 */
	public static void manageActivationConditions(Goal goal, Goal newGoal, List<List<String>> successConditionsSplitted) {
		for (List<String> conds : successConditionsSplitted) { 
			boolean more = true;
			if (more) {
				Set<String> dependencies = Queries.getDependenciesForElement(new HashSet<String>(conds));
				
				for (EObject act : goal.getActivationConditions()) { //Example: ACT(EDEF(Episode.patient))
					if (act instanceof EntityAchieveCondition) {
						...
						for (String dependence : dependencies){
							if (dependence.equals(o.getName())) {
								newGoal.getActivationConditions().add(copy(act));
							}	
						}
					} //if
						...
						for (String c : o.getConditions()) {
							for (String dependence : dependencies){
								if (dependence.equals(c)) { //if any of the elements in the group has a dependence => we include the ACT
									EObject actCopy = copy(act);
									if (!newGoal.getActivationConditions().contains(actCopy)) {
										if (more) {
											newGoal.getActivationConditions().add(copy(act));
											more = false;
										}
	...
	/**
	 * I just need to call to the engine to know given a SUC which invariants it has 
	 * That way I will copy such MUL/RULs with the corresponding SUCs
	 * @param goal
	 * @param newGoal
	 * @param serializedSuccessConditions
	 */
	public static void manageInvariantConditions(Goal goal, Goal newGoal, List<List<String>> successConditionsSplitted) {
		for (List<String> conds : successConditionsSplitted) { 
			boolean more = true;
			if (more) {
				for (EObject inv : goal.getInvariantConditions()) { 
					if (inv instanceof EntityInvariantCondition) { //Example: MUL(Episode.report, 1)
						EntityInvariantCondition o = (EntityInvariantCondition)inv; 
						if (conds.size() == 1) //Its and EntityInvariant -> There should be only an element
							if (Queries.getEntityNameFrom(o.getName()).equals(conds.get(0))) { 
						...
						List<String> list = new ArrayList<String>();
						Queries.getDecomposedExpression(o.getExpression(), list);
						Set<String> dependencies = Queries.getAttributesRelatedToRule(new HashSet<String>(list));
						for (String c : conds) { //if any of the elements in the group has a dependence => we include the RUL
							for (String dependence : dependencies){
								if (dependence.equals(c)) { 
									EObject invCopy = copy(inv);
									if (!newGoal.getInvariantConditions().contains(invCopy)) {
										if (more) {
											newGoal.getInvariantConditions().add(copy(inv));	
											more = false;
										}										
	...
	public static Optional<ValueException> manageRelatives(GoalModel model, Goal goal, Goal newGoal1, Goal newGoal2, String type) {	
		Optional<ValueException> result = Optional.empty();
		switch (type) {
			case "s":
			case "sibling": //the new node is a sibling of the previous one
				//we want the two nodes (g1 and g2) that are together now (g) become siblins
				//we need to check whether g1 and g2 can be splitted. They cannot if SUC(g2) is an attribute of (g1)
				if(!isNewSiblingPossible(newGoal1, newGoal2)) 
					result = Optional.of(new ValueException());
				else {
					changeInputReferencesSibling(model, goal, newGoal1, newGoal2); //instead to GOAL, to NEWGOAL1 and NEWGOAL2
					changeOutputReferencesSibling(goal, newGoal1, newGoal2); //changing references from GOAL to be from NEWGOAL1 or NEWGOAL2 instead
				}
				break;
			case "c":
			case "child": //the new node is the child of the previous one
				//we want the two nodes (g1 and g2) that are together now (g) become parent (g1) and child (g2)
				
								
				//OK -> we do (newGoal2 refers now to newGoal1 - newGoal2 contains the SUC that have been specified in the command line)
	...
	public static void changeInputReferencesSibling(GoalModel model, Goal goal, Goal newGoal1, Goal newGoal2) {
		//INPUT REFERENCES
		for (Goal g : model.getGoals()) { //for each of the goals
			if (g.getChildrenGoals().contains(goal)) { //we need to study to which one is the reference to make changes
				for(EObject suc : g.getSuccessConditions()) {
					if (suc instanceof EntityAchieveCondition) {
						EntityAchieveCondition o = (EntityAchieveCondition)suc;
						//if (o is an entity) && (newGoal1 is an attribute of that entity) --> 
						newGoal1.getSuccessConditions().stream()
							...
							.forEach(e -> { 
								...
								att.getConditions().stream()
									.map(a -> Queries.getEntityNameFrom(a))
									.forEach(a -> {
										if ((a.equals(o.getName())&&(!g.getChildrenGoals().contains(newGoal1)))) 
											g.getChildrenGoals().add(newGoal1);
									});
							});
						//if (o is an entity) && (newGoal2 is an attribute of that entity) -->
						...
						.forEach(e -> { 
							...
							att.getConditions().stream()
								.map(a -> Queries.getEntityNameFrom(a))
								.forEach(a -> {
									if ((a.equals(o.getName())&&(!g.getChildrenGoals().contains(newGoal2)))) 
					...
					//}
						...
						//if (o is nothing) --> both of them if there have Entities
						if (newGoal1.getSuccessConditions().stream()
								.filter(e -> e instanceof EntityAchieveCondition)
								.findFirst().isPresent())
							if (!g.getChildrenGoals().contains(newGoal1)) g.getChildrenGoals().add(newGoal1);
						if (newGoal2.getSuccessConditions().stream()
								.filter(e -> e instanceof EntityAchieveCondition)
								.findFirst().isPresent())
							if (!g.getChildrenGoals().contains(newGoal2)) g.getChildrenGoals().add(newGoal2);
					}
				} //for	
				if ((!g.getChildrenGoals().contains(newGoal1))&&(!g.getChildrenGoals().contains(newGoal2))) {
					//we didn't insert the dependency yet but we need at least one --> ask the user or use by default one of them
					g.getChildrenGoals().add(newGoal2);
				}
	...
	public static void changeOutputReferencesSibling(Goal goal, Goal newGoal1, Goal newGoal2) {
		//OUTPUT REFERENCES
		for (Goal g : goal.getChildrenGoals()) { //output references from the original node
			for (EObject suc : g.getSuccessConditions()) { //to identify the SUCs of the goals from goal
			    if (suc instanceof NotMandatoryAttributeAchieveCondition) {
					NotMandatoryAttributeAchieveCondition o = (NotMandatoryAttributeAchieveCondition)suc;	
					...
					.forEach(e -> { 
						...
						o.getConditions().stream()
							.map(a -> Queries.getEntityNameFrom(a))
							.forEach(a -> {
								if ((a.equals(ent.getName())&&(!newGoal1.getChildrenGoals().contains(g)))) 
					...
					.forEach(e -> { 
						...
						o.getConditions().stream()
							.map(a -> Queries.getEntityNameFrom(a))
							.forEach(a -> {
								if ((a.equals(ent.getName())&&(!newGoal2.getChildrenGoals().contains(g)))) 
									newGoal2.getChildrenGoals().add(g);
							});
					});
				}
			} //for
			if ((!newGoal1.getChildrenGoals().contains(g))&&(!newGoal2.getChildrenGoals().contains(g))) {
				//we didn't insert the dependency yet but we need at least one --> ask the user or use by default one of them
				newGoal2.getChildrenGoals().add(g);
			}
			g.getChildrenGoals().remove(goal);
	...
	public static boolean isNewSiblingPossible(Goal newGoal1, Goal newGoal2) {
		...
		for (EObject obj1 : newGoal1.getSuccessConditions()) {
			for (EObject obj2 : newGoal2.getSuccessConditions()) {
				if ((obj1 instanceof EntityAchieveCondition)&&(obj2 instanceof NotMandatoryAttributeAchieveCondition)) {
					...
					for (String c : o2.getConditions()) {
						if (Queries.getEntityNameFrom(c).equals(o1.getName())) return false;
					}
				}
				if ((obj2 instanceof EntityAchieveCondition)&&(obj1 instanceof NotMandatoryAttributeAchieveCondition)) {
					...
					for (String c : o1.getConditions()) {
						if (Queries.getEntityNameFrom(c).equals(o2.getName())) return false;
					}
				}
			} 
		}
	...
	public static void changeParentReferencesChild(GoalModel model, Goal goal, Goal newGoal1, Goal newGoal2) {
	...
	private boolean isPossibleToSplit(GoalModel goal, Goal source, Goal newGoal1, Goal newGoal2) {
