...
public class ManageLs {
    public static void goals(GoalModel model, String name, CommandLs cmd) {
        ...
        ICompositeNode node = NodeModelUtils
                .getNode(model.eResource().getContents().get(0));
        ConsoleManagement.write(name, ConsoleManagement.TypeOutput.OutputData,
                node.getText());
    }

    // public static void entities(DataModel model, String name, CommandLs cmd)
    ...
    // if (ent.getDependsOn() != null) textEnt += " dependsOn " +
    // ent.getDependsOn().getName();
    // ConsoleManagement.write(name, ConsoleManagement.TypeOutput.OutputData,
    // textEnt);
    ...
    // if (att.getDependsOn().size() > 0) textAtt += " dependsOn " +
    // att.getDependsOn()
    // .stream()
    // .map(element -> ((Entity)element.eContainer()).getName() + "." +
    // element.getName())
    ...
    // if (attG.getDependsOn().size() > 0) textAttG += " dependsOn " +
    // attG.getDependsOn()
    // .stream()
    // .map(element -> ((Entity)element.eContainer()).getName() + "." +
    // element.getName())
