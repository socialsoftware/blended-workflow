       ...
       return (
           <Form inline>
                ...
                <DropdownButton 
                    ...
                    id='2'>
                    <MenuItem eventKey="1" onClick={() => this.setOperation(OPERATION.RENAME)}>{OPERATION.RENAME}</MenuItem>
                    <MenuItem eventKey="2" onClick={() => this.setOperation(OPERATION.MERGE)}>{OPERATION.MERGE}</MenuItem>
                    <MenuItem eventKey="3" onClick={() => this.setOperation(OPERATION.SPLIT)}>{OPERATION.SPLIT}</MenuItem>
                </DropdownButton></span>}{' '}

                {this.props.mergeWithGoal.name && <span>
                <Button id='3'>{this.props.mergeWithGoal.name}</Button></span>}

                {this.state.operation === OPERATION.SPLIT && this.props.goalConditions &&
                <DropdownButton 
                    bsStyle='default'
                    title={'conditions'}
                    id='4'>
                ...
                {(this.state.operation === OPERATION.RENAME || this.props.mergeWithGoal.name || 
